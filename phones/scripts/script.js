...
class Phone {
    ...
    createElement(element) {
        ...
        if (element.price.oldUan && element.price.newUan) {
            oldprice.classList = 'oldPrice'
            oldprice.textContent = `${element.price.oldUan}`
            div.append(oldprice)
            price.classList = 'newPrice'
            price.textContent = `${element.price.newUan}`
            div.append(price)
        } else {
        ...
        p.textContent = `${element.ratingRevievs}`
        div.append(p);
        return div;
    }
    ...
    originalMap() {
        ...
        temp.map(element => {
            fragment.append(this.createElement(element))
        })
        phones.innerHTML = "";
        phones.append(fragment);
    }
    ...
    sortMap() {
        ...
        temp.sort(function (a, b) {
            if (a.price.newUan && b.price.newUan) {
                let temp_a = Number(a.price.newUan.replace(/\D+/g,""));
                let temp_b = Number(b.price.newUan.replace(/\D+/g,""))
                if (temp_a > temp_b) {
                    return 1;
                }
            ...
            if (a.price.newUan && !b.price.newUan) {
                let temp_a = Number(a.price.newUan.replace(/\D+/g,""));
                let temp_b = Number(b.price.replace(/\D+/g,""))
                if (temp_a > temp_b) {
                    return 1;
                }
                else if (temp_a < temp_b) {
            ...
            if (!a.price.newUan && b.price.newUan) {
                let temp_a = Number(a.price.replace(/\D+/g,""));
                let temp_b = Number(b.price.newUan.replace(/\D+/g,""))
                if (temp_a > temp_b) {
                    return 1;
                }
...
var data = [{ratingRevievs: "264 отзыва", price: {oldUan: "4 333 грн", newUan: "3 799 грн"}, name: "Motorola MOTO G4 (XT1622) Black"}, 
...
sort.textContent = "Отсортировать по цене";
sort.setAttribute("onclick", "phone.sortMap()");
header.appendChild(sort);

let original = document.createElement("button");
original.textContent = "Исходный массив";
original.setAttribute("onclick", "phone.originalMap()");
